...
const open = (assessor: User | null) => {
    ...
    form._method = 'PUT';
    form.id = assessor.id;
    form.name = assessor.name;
    form.email = assessor.email;
    form.phone = assessor.phone;
};
...
const submit = () => {
    const url = form.id === 0 ?
        route('assessor.store') :
        route('assessor.update', form.id);
    form.post(url, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: close,
...
<template>
    <Dialog
        ...
        @after-hide="close" :header="$t('menu.assessor')" :style="{ width: '30rem' }">
