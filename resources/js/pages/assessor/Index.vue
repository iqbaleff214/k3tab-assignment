...
const destroy = (event: MouseEvent, item: User | null) => {
    confirm.require({
        ...
        accept: () => {
            const url = !item ?
                route('assessor.mass-destroy') :
                route('assessor.destroy', item?.id);
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            <header class="flex items-center justify-between">
                <div>
                    <h3 class="mb-0.5 text-base font-medium">{{ $t('menu.assessor') }}</h3>
                    <p class="text-sm text-muted-foreground">{{ $t('label.menu_subtitle', { menu: $t('menu.assessor') }) }}</p>
                </div>
                <div class="flex gap-x-2">
                    ...
                    <template v-else>
                        <Button
                            ...
                            :label="$t('action.new_menu', { menu: $t('menu.assessor') })" size="small" />
                    </template>
            ...
            <DataTable
                name="assessor_table" :selection="true"
