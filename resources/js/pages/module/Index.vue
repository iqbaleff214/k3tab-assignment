...
const searchTitle = (value: string | undefined): void => {
        ...
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            <header class="flex items-center justify-between">
                ...
                <div class="flex gap-x-2">
                    <div>
                        <InputText
                            placeholder="Search by title" :fluid="true" v-model="filterForm.filters.title.value" size="small"
                            type="text" autocomplete="off" @update:model-value="searchTitle" />
                    </div>
            ...
            <DataTable
                name="module_table" :selection="true"
                v-model:selected="selected"
                :items="items.data">
                <Column field="title" header="field.title" :sortable="true">
                    <template #body="{ row }: { row: Module }">
                        <Link :href="route('admin.module.show', row.id)" class="hover:underline hover:text-amber-500">
                            {{ row.title }}
                        </Link>
                    </template>
                </Column>
                <Column field="minimum_score" header="field.minimum_score" :sortable="true" />
                <Column field="questions_count" header="field.questions_count" :sortable="true">
                    <template #body="{ row }: { row: Module }">
                ...
                <Column field="duration_estimation" header="field.duration_estimation" :sortable="true">
                    <template #body="{ row }: { row: Module }">
                        <span class="text-xs text-gray-500" v-if="!row.duration_estimation" v-text="$t('label.no_estimation')"></span>
                        <span v-else v-text="row.duration_estimation"></span>
                    </template>
                </Column>
